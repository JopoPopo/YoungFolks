...
with st.sidebar:
    ...
    if uploaded_file_ui and "doc_ref" not in st.session_state:
        with st.spinner("Uploading your document..."):
            # here, we check the file type
            mime_type = uploaded_file_ui.type

            # writing bytes to temp file
            with open("temp_doc", "wb") as f: # creating temp file with writing privileges
...
if "messages" not in st.session_state:
    st.session_state.messages = [] # creating a list of dictionaries where each dict is a message containing role and content

for msg in st.session_state.messages: # iterate through list of dicts
    with st.chat_message(msg["role"]): # create a div block for each role
        st.markdown(msg["content"]) # print the content of the message
